...
const PopularItems = () => {
    ...
    useEffect(() => {
        const fetchMenuData = async () => {
            try {
                const response = await fetch('https://restaurantapi.fatmonk.studio/api/restaurant/crimsoncup/popular/products');
    ...
    return (
        <div className='grid grid-cols-2 gap-4 px-4 sm:px-8 md:mx-52 lg:mx-60 my-4'>
                    ...
                    <div className="h-full bg-cover bg-center" style={{ backgroundImage: `url(${menuItem.product_picture})` }}>
                        ...
                        <div className="flex items-center">
                            <div className='p-2 z-50'>
                                ...
                                <p className="[font-family:'Montserrat-Regular',Helvetica] font-normal text-white text-[10px] sm:text-[12px] md:text-[18px] lg:text-[22px] tracking-[0] leading-[normal] whitespace-nowrap mt-[2px]">{menuItem.product_status}</p>
                            </div>
                        </div>
            ...
            <Modal isOpen={isModalOpen} closeModal={closeModal} menuData={selectedMenuItem} />
        </div>
    );
};
